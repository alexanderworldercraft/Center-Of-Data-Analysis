...
exports.assertNamingConvention = assertNamingConvention;
const utils_1 = require("../../utils");
function assertNamingConvention(exportValue, exportName, telefuncFilePath, appRootDir) {
...
function assertStartsWithOn(exportName, telefuncFilePath) {
    ...
    if (!/on/.test(exportName)) {
        (0, utils_1.assertWarning)(false, `We recommend the name of your telefunction ${exportName}() (${telefuncFilePath}) to start with "on", see https://telefunc.com/event-based#naming-convention'`, { onlyOnce: true });
    }
    else {
        (0, utils_1.assertWarning)(/on[A-Z]/.test(exportName), `The name of your telefunction ${exportName}() (${telefuncFilePath}) starts with "on" but isn't followed by a capital letter, see https://telefunc.com/event-based#naming-convention'`, { onlyOnce: true });
    }
}
function assertCollocation(telefuncFilePath, appRootDir, exportValue) {
    ...
    const getBasename = (fileNameOrPath) => {
        (0, utils_1.assertPosixPath)(fileNameOrPath);
    ...
    collocatedFiles.forEach((fileName) => {
        (0, utils_1.assertPosixPath)(fileName); // fileName isn't a path so it shouldn't contain any backslash windows path separator
    ...
    */
    (0, utils_1.assertWarning)(collocatedFilesMatchYes.length >= 2, [
