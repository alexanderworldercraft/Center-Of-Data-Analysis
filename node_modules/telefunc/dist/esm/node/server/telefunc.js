...
function assertHttpRequest(httpRequest, numberOfArguments) {
    ...
    assertUsage(!('context' in httpRequest) || hasProp(httpRequest, 'context', 'object'), '`telefunc({ context })`: argument `context` should be an object.');
    Object.keys(httpRequest).forEach((key) => {
        assertUsage(['url', 'method', 'body', 'context'].includes(key), '`telefunc({ ' + key + ' })`: Unknown argument `' + key + '`.');
    });
